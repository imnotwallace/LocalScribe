...
- **Recording pause:** worker subscribes to the same recording-active signal `AssistantGate` uses; on recording start it finishes the in-flight batch (seconds), then parks **and kills the warm embed helper process** so its memory returns to the live pipeline; resumes (respawning the helper, ~2s model reload) on recording end. Query embedding is exempt — mid-recording queries respawn the helper on demand.
- **Memory posture (32GB business-laptop rule):** the 4B chat LLM is NEVER loaded by any semantic-search path — the embed op loads only EmbeddingGemma-300m (~300MB quantized; ~600MB process working set). At most ONE warm embed helper process exists (shared by backfill and queries, serialized), it is killed on recording start and by the embed client's own 5-minute idle reclaim (added at final review: the pre-existing watchdog is a mid-request hang guard, not an idle reclaim). Steady state outside active indexing/searching: zero extra resident processes.
