...
public static class MarkdownRenderer
{
    ...
    public static string Render(TranscriptHeader header, IReadOnlyList<DisplayRow> rows, string timestampsMode)
    {
        ...
        sb.Append(header.App).Append(Dot)
          ...
          .Append(durationMin.ToString(CultureInfo.InvariantCulture)).Append(" min");
        AppendEngine(sb, header);
        sb.Append('\n');
        sb.Append('\n');
    ...
    /// <summary>The engine segment of the header line. 2026-08-11 final review M-b: this used to
    /// be an unconditional "{Model}/{Backend}", so a session that never transcribed anything -
    /// a SALVAGED import is exactly that case, and its transcript.md is regenerated BY the salvage
    /// path - rendered a bare "Manual - 2026-08-11 14:32 - 5 min - /". session.json leaves
    /// Model/Backend empty precisely so no engine is CLAIMED; the header must honour that by
    /// omitting the segment rather than rendering the separator around two blanks. A session with
    /// only one of the two known still renders it, since that half is a real claim.
    /// PlainTextRenderer carries the identical rule for transcript.txt.</summary>
    private static void AppendEngine(StringBuilder sb, TranscriptHeader header)
    {
        if (string.IsNullOrEmpty(header.Model) && string.IsNullOrEmpty(header.Backend)) return;
        sb.Append(Dot).Append(header.Model).Append('/').Append(header.Backend);
    }
