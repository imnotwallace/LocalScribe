...
- Produces: `ReassignSpeakerViewModel(MaintenanceService, IUiErrorReporter, string sessionId, TranscriptSource source, IReadOnlyList<RowSegment> segments, SessionMeta meta, Speakers? speakers, string timestampsMode, DateTimeOffset startedAtLocal)` with `Candidates : IReadOnlyList<SpeakerCandidate>` (participants first, then unowned named clusters), `HasCandidates`, `Segments : IReadOnlyList<SegmentChoiceViewModel>` (checkbox items; other-stream segments disabled), `SelectedCandidate`, `ValidationMessage`, `IsSaving`, `Task<bool> SaveAsync(CancellationToken)`, `event Action<string>? OpenSessionDetailsRequested`, `void RequestOpenSessionDetails()`. `SpeakerCandidate(string Display, SpeakerPinTarget Target)`.
...
public sealed record SpeakerCandidate(string Display, SpeakerPinTarget Target);
...
public sealed partial class ReassignSpeakerViewModel : ObservableObject
{
    ...
    public IReadOnlyList<SpeakerCandidate> Candidates { get; }
    public IReadOnlyList<SegmentChoiceViewModel> Segments { get; }
    public bool HasCandidates => Candidates.Count > 0;
    [ObservableProperty] private SpeakerCandidate? _selectedCandidate;
    [ObservableProperty] private bool _isSaving;
    [ObservableProperty] private string _validationMessage = "";
        ...
        SessionMeta meta, Speakers? speakers, string timestampsMode, DateTimeOffset startedAtLocal)
    {
        ...
        foreach (var p in meta.Participants)
        {
            if (p.Side != side || p.Kind != ParticipantKind.Named || string.IsNullOrEmpty(p.Name))
                continue;
            candidates.Add(new SpeakerCandidate(p.Name, new SpeakerPinTarget.Participant(p.Id)));
        ...
        if (speakers is not null)
        {
            ...
            foreach (var (key, name) in speakers.Names)
                if (key.StartsWith(prefix, StringComparison.Ordinal) && !ownedKeys.Contains(key))
                    candidates.Add(new SpeakerCandidate($"{name} (detected voice)",
